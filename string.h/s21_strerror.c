...
void s21_errnum_tostring(char *str, int num) {
  ...
  part[i] = '\0';
  for (s21_size_t i = 0; i < s21_strlen(part); i++) {
    if (flag == 0)
      str[i] = part[s21_strlen(part) - i - 1];
    else
      str[i + 1] = part[s21_strlen(part) - i - 1];
  }
  if (flag == 1) {
    str[0] = '-';
    str[s21_strlen(part) + 1] = '\0';
  } else
    str[s21_strlen(part)] = '\0';
}
